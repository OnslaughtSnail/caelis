...
// MutationPreview is one dry-run file mutation preview computed against the
// current filesystem snapshot before the tool executes.
...
func BuildMutationPreview(runtime toolexec.Runtime, toolName string, args map[string]any) (MutationPreview, error) {
	resolvedRuntime, err := runtimeOrDefault(runtime)
	if err != nil {
		return MutationPreview{}, err
	}

	plan, err := buildMutationPlan(resolvedRuntime.FileSystem(), toolName, args)
	if err != nil {
		return MutationPreview{}, err
	}
	return MutationPreview{
		...
		New:     plan.after,
		Hunk:    plan.hunk,
		Preview: buildPatchPreview(plan.before, plan.after),
	}, nil
}

func buildMutationPlan(fsys toolexec.FileSystem, toolName string, args map[string]any) (fileMutationPlan, error) {
	switch strings.ToUpper(strings.TrimSpace(toolName)) {
	case PatchToolName:
		return planPatchMutation(fsys, args)
	case WriteToolName:
		return planWriteMutation(fsys, args)
	default:
		return fileMutationPlan{}, fmt.Errorf("tool: %s does not support diff preview", strings.TrimSpace(toolName))
	}
}

func planWriteMutation(fsys toolexec.FileSystem, args map[string]any) (fileMutationPlan, error) {
...
func planPatchMutation(fsys toolexec.FileSystem, args map[string]any) (fileMutationPlan, error) {
	pathArg, err := argparse.String(args, "path", true)
	if err != nil {
		return fileMutationPlan{}, err
	}
	...
	if !exists {
		return fileMutationPlan{}, fmt.Errorf("tool: missing required arg %q", "old")
	}
	oldValue, ok := rawOld.(string)
	if !ok {
		return fileMutationPlan{}, fmt.Errorf("tool: arg %q must be string", "old")
	}
