...
func NewToolUsePart(id, name string, input json.RawMessage) Part {
	return Part{Kind: PartKindToolUse, ToolUse: &ToolUsePart{ID: id, Name: name, Input: append(json.RawMessage(nil), input...)}}
}
...
func NewFileRefPart(name, mimeType, uri, fileID, localRef string) Part {
	return Part{Kind: PartKindFileRef, FileRef: &FileRefPart{Name: name, MimeType: mimeType, URI: uri, FileID: fileID, LocalRef: localRef}}
}
...
func MessageFromToolResponse(resp *ToolResponse) Message {
	...
	return Message{Role: RoleTool, Parts: []Part{NewToolResultJSONPart(resp.ID, resp.Name, resp.Result, false)}}
}
...
// ToolDefinition remains as the concrete function-tool declaration used by the
...
// Response is the completed semantic result of one model step/turn.
...
// StreamEvent is the unified typed event stream emitted by all providers.
...
func cloneReasoningPart(part *ReasoningPart) *ReasoningPart {
	...
	cp.Replay = cloneReplayMeta(part.Replay)
	cp.ProviderDetails = cloneRawMessageMap(part.ProviderDetails)
	return &cp
}
...
func cloneReplayMeta(meta *ReplayMeta) *ReplayMeta {
	if meta == nil {
		return nil
	}
	cp := *meta
