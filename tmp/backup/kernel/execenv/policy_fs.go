...
func newPolicyFileSystem(base FileSystem, policy func() SandboxPolicy) FileSystem {
	if base == nil || policy == nil {
		return base
	}
	return &policyFileSystem{base: base, policy: policy}
}
...
func (f *policyFileSystem) checkWritePath(path string) error {
	policy := f.policy()
	switch policy.Type {
	case SandboxPolicyDangerFull, SandboxPolicyExternal:
		return nil
	...
	if fsboundary.IsWithinReadOnlySubpaths(targetPath, policy.ReadOnlySubpaths, f.base) {
		return permissionError("write", targetPath, "path is under read-only sandbox subpath")
	}
	if fsboundary.IsWithinRoots(targetPath, policy.WritableRoots, f.base) || fsboundary.IsWithinScratchRoots(targetPath, f.base) {
		return nil
	}
...
func (f *policyFileSystem) checkReadPath(path string) error {
	...
	if fsboundary.IsWithinRoots(targetPath, policy.ReadableRoots, f.base) ||
		fsboundary.IsWithinRoots(targetPath, policy.WritableRoots, f.base) ||
...
func globReadRoot(pattern string, fsys FileSystem) string {
