...
func RecoveredBashPreview(session toolexec.Session) string {
	output := ReadRetainedOutput(session)
	return BashOutputPreview([]byte(output.Stdout), []byte(output.Stderr))
}
...
func bashTaskLikelyWaitingInput(preview string) bool {
	...
	return len(last) <= 120 && (strings.HasSuffix(last, ":") || strings.HasSuffix(last, "?"))
}
