...
// TurnHandle is the application-layer wrapper around a single running turn.
// It intentionally mirrors the runtime runner contract so callers no longer
// need to depend on kernel/runtime directly.
...
func (s *Service) RunTurn(ctx context.Context, req RunTurnRequest) (RunTurnResult, error) {
	...
	if active := s.activeTurn(ref.SessionID); active != nil {
		return RunTurnResult{}, fmt.Errorf("sessionsvc: session %q already has an active run", ref.SessionID)
	}
	runSvc, err := apprunservice.New(apprunservice.ServiceConfig{
		Runtime:               s.runtime,
		AppName:               ref.AppName,
		UserID:                ref.UserID,
	...
	runResult, err := runSvc.RunTurn(runCtx, apprunservice.RunTurnRequest{
		SessionID:           ref.SessionID,
		Input:               req.Input,
	...
	s.setActive(ref.SessionID, &activeTurn{
		cancel: cancel,
		handle: handle,
	})
...
func (s *Service) setActive(sessionID string, active *activeTurn) {
...
func isSessionNotFound(err error) bool {
	return err != nil && strings.Contains(err.Error(), session.ErrSessionNotFound.Error())
}
