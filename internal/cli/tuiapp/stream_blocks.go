...
func (m *Model) handleRawDelta(msg tuievents.RawDeltaMsg) (tea.Model, tea.Cmd) {
	switch msg.Target {
	case tuievents.RawDeltaTargetBTW:
		return m.enqueueBTWDelta(msg.Text, msg.Final)
	case tuievents.RawDeltaTargetSubagent:
...
func (m *Model) flushSpawnPreviewSmoothingExcept(panelID string, streamKind string) {
	m.flushMatchingStreamSmoothing(func(state *streamSmoothingState) bool {
		return state != nil && state.targetKind == "spawn_preview" && state.sessionKey == panelID && state.streamKind != streamKind
	})
}
...
func (m *Model) handleToolStreamMsg(msg tuievents.TaskStreamMsg) (tea.Model, tea.Cmd) {
	...
	if strings.TrimSpace(msg.Chunk) != "" {
		...
		cmd = m.ensurePendingStreamSmoothingTick()
	}
	if msg.Final && isSpawnLikeTool(panel.ToolName) {
		m.flushMatchingStreamSmoothing(func(state *streamSmoothingState) bool {
			return state != nil && state.targetKind == "spawn_preview" && state.sessionKey == panel.BlockID()
		})
	}
	...
	if msg.Final && isTerminalToolOutputState(panel.State) {
		...
		if isInlineBashPanel(panel) {
			m.scheduleInlineBashCollapse(panel)
			if animCmd := m.ensurePanelAnimationTick(); animCmd != nil {
